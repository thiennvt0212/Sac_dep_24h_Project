...
>
  ...
  <div class="max-w-[1550px] mx-auto px-[20px]">
    <div class="overflow-hidden max-w-[1380px] w-full mx-auto relative px-[30px]">
      ...
      >
        ...
        >
          ...
          <div
            ...
            :class="{ 'border-pink-600 scale-95': selectedCategory === index, 'border-pink-300': selectedCategory !== index }"
          >
           <img
  :src="category.image"
...
/>
      ...
      <!-- Thanh công cụ -->
      <div class="w-[1360px] mx-auto flex flex-wrap justify-between items-center border-b border-gray-300 pb-[15px] mb-6">
        <div class="text-sm text-gray-700 mb-2 sm:mb-0">
            Hiển thị {{ startResult }}–{{ endResult }} của {{ products.length }} kết quả
        </div>
        <div class="flex gap-2 items-center">
          ...
          <div class="flex gap-2 items-center">
            ...
            <button
              ...
              :class="{ 'border-pink-600 bg-pink-100 text-pink-600': viewMode === 'grid2' }"
            ...
            <button
              class="w-10 h-10 rounded-lg border border-gray-300 hover:border-pink-500 flex items-center justify-center transition-all duration-300"
              :class="{ 'border-pink-600 bg-pink-100 text-pink-600': viewMode === 'grid3' }"
            ...
            <button
              class="w-10 h-10 rounded-lg border border-gray-300 hover:border-pink-500 flex items-center justify-center transition-all duration-300"
              :class="{ 'border-pink-600 bg-pink-100 text-pink-600': viewMode === 'grid4' }"
...
>
          <div class="relative">
            <div class="w-full aspect-[1.026] overflow-hidden rounded-t-xl bg-white">
                <img
                  :src="product.image"
                  alt="Tên sản phẩm"
            ...
            >
              <button
                ...
                title="Thêm vào giỏ hàng"
              >
                <span class="group-hover/cart:hidden block transition duration-200">Thêm vào giỏ hàng</span>
                <span class="hidden group-hover/cart:inline transition duration-200">🛒</span>
              </button>
            </div>
          </div>
          ...
          <div class="p-4 text-center">
            ...
            <div class="text-red-500 font-bold mt-1">
              {{ product.price }} đ
              <span v-if="product.oldPrice" class="text-xs text-gray-400 line-through ml-2">
                {{ product.oldPrice }} đ
              </span>
            </div>
          </div>
  ...
  <div class="w-[1360px] mx-auto py-10 flex justify-center space-x-2">
    <button
      ...
      :class="[
        ...
        currentPage === page ? 'bg-pink-200 text-white font-bold' : 'bg-white text-gray-700'
      ]"
    >
      {{ page }}
    </button>
  ...
  endResult() {
    return Math.min(this.currentPage * this.productsPerPage, this.products.length);
  }
  },
    methods: {
  scrollLeft() {
